...
DeviceTypeSchema.pre("save", function (next) {
  if (this.isModified("isActive") && (this as any).isActive === false) {
    if (!(this.get("name") as string).includes("_deleted_")) {
      const timestamp = Date.now();
      this.set("name", `${this.get("name")}_deleted_${timestamp}`);
    }
  }

  if (
    this.isModified("validRecipeStepNames") &&
    (this as any).validRecipeStepNames
  ) {
    const trimmed = (this.get("validRecipeStepNames") as string[])
      .map((name) => name.trim())
      .filter((name) => name.length > 0);
    this.set("validRecipeStepNames", trimmed);
  }
