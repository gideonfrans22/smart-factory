...
export class RecipeService {
  ...
  async getDependencyGraph(id: string): Promise<{
    ...
    (recipe as any).steps.forEach((step: any) => {
      const stepIdStr = step._id.toString();
      graph[stepIdStr] = (step.dependsOn || []).map((x: any) => x.toString());
    ...
    while (queue.length > 0) {
      ...
      (recipe as any).steps.forEach((s: any) => {
        const dependsOnStr = (s.dependsOn || []).map((x: any) => x.toString());
    ...
    return {
      ...
      recipeNumber: (recipe as any).recipeNumber,
      version: (recipe as any).version,
      topologicalOrder,
