...
/**
 * Get tasks grouped by device
 * GET /api/tasks/by-device
 */
export const getTasksByDevice = async (
  _req: Request,
  res: Response
): Promise<void> => {
  try {
    // Get all devices with tasks
    const devices = await Task.aggregate([
      {
        $match: {
          deviceId: { $ne: null }
        }
    ...
    // Populate device details and calculate metrics
    const tasksByDevice = await Promise.all(
      devices.map(async (deviceGroup) => {
        const device = await require("../models/Device").Device.findById(
          deviceGroup._id
        ).populate("deviceTypeId", "name");
        ...
        return {
          ...
          deviceType: (device.deviceTypeId as any)?.name,
          status: device.status,
          tasks: deviceGroup.tasks.map((t: any) => ({
            taskId: t.taskId,
            title: t.title,
            status: t.status,
            progress: t.progress || 0,
            estimatedTimeRemaining: t.status === "ONGOING" 
              ? Math.round((t.estimatedDuration || 0) - (Date.now() - new Date(t.createdAt).getTime()) / 60000)
              : Math.round(t.estimatedDuration || 0)
          })),
          queueLength: deviceGroup.pendingCount,
          ongoingCount: deviceGroup.ongoingCount,
          avgWaitTimeMinutes: avgWaitTime,
          totalTasksCount: deviceGroup.tasks.length
        };
      })
    );

    // Filter out null devices
    const filteredDevices = tasksByDevice.filter((d) => d !== null);

    const response: APIResponse = {
      success: true,
      message: "Tasks grouped by device retrieved successfully",
      data: {
        items: filteredDevices,
        summary: {
          totalDevices: filteredDevices.length,
          totalTasks: filteredDevices.reduce((sum, d) => sum + d.totalTasksCount, 0),
          totalQueueLength: filteredDevices.reduce((sum, d) => sum + d.queueLength, 0),
          avgQueueLength: Math.round(
            filteredDevices.reduce((sum, d) => sum + d.queueLength, 0) / 
            Math.max(1, filteredDevices.length)
          ),
          avgWaitTime: Math.round(
            filteredDevices.reduce((sum, d) => sum + d.avgWaitTimeMinutes, 0) /
            Math.max(1, filteredDevices.length)
          )
        }
      }
    ...
    console.error("Get tasks by device error:", error);
    const response: APIResponse = {
      success: false,
      error: "INTERNAL_SERVER_ERROR",
      message: "Internal server error"
...
/**
 * Get bottleneck analysis - identify slowest processes
 * GET /api/tasks/bottleneck-analysis
 */
export const getBottleneckAnalysis = async (
  ...
  try {
    // Analyze tasks by device type to find bottlenecks
    const deviceTypeAnalysis = await Task.aggregate([
      {
        $match: {
    ...
    const bottlenecks = await Promise.all(
      deviceTypeAnalysis.map(async (dt) => {
        const queueLength = dt.pendingCount;
        ...
        const avgWaitMinutes = Math.round((avgWaitTime[0]?.avgWait || 0) / 60000);
        const impactScore = Math.min(
          100,
          (queueLength * 10 + avgWaitMinutes * 2 + dt.avgDuration / 10)
        );

        return {
          deviceTypeId: dt._id,
          deviceTypeName: dt.deviceType.name,
          avgTaskDuration: Math.round(dt.avgDuration || 0),
          avgWaitTime: avgWaitMinutes,
          queueLength,
          impactScore: Math.round(impactScore),
          recommendation:
            impactScore > 70
              ? "Add parallel processing or additional resources"
              : impactScore > 50
              ? "Monitor queue length and optimize workflow"
    ...
    // Find slowest step (aggregated across all tasks)
    const slowestStep = await Task.aggregate([
      {
        $match: {
          status: "COMPLETED"
        }
    ...
    // Calculate bottleneck rate (% of total queue vs capacity)
    const totalQueueLength = bottlenecks.reduce((sum, b) => sum + b.queueLength, 0);
    const totalCapacity = bottlenecks.reduce(
      (sum, b) => sum + (b.avgTaskDuration ? 60 / b.avgTaskDuration : 1),
      0
    );
    const bottleneckRate = Math.round((totalQueueLength / Math.max(1, totalCapacity)) * 10000) / 100;

    // Estimate production loss
    const estimatedProductionLoss =
      Math.round(
        ((bottlenecks[0]?.avgWaitTime || 0) / 60 / (bottlenecks[0]?.avgTaskDuration || 60)) * 100
      ) / 100;
    ...
    const response: APIResponse = {
      ...
      data: {
        bottlenecks,
        slowestStep: {
          stepId: slowestStep[0]?._id || "unknown",
          stepName: `Step ${slowestStep[0]?._id || "N/A"}`,
          avgDuration: Math.round(slowestStep[0]?.avgDuration || 0),
          percentageOfTotal: slowestStep[0]
            ? Math.round(
                ((slowestStep[0].avgDuration || 0) /
                  (deviceTypeAnalysis.reduce((sum, d) => sum + d.avgDuration, 0) /
                    deviceTypeAnalysis.length)) *
                  10000
              ) / 100
            : 0
        },
        overallBottleneckRate: bottleneckRate,
